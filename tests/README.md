...
camera, mission, battery, thermal, tyre/brake thermal, steering-feel and wear tests, all
on the tick clock (`--fixed-fps 60`), under a minute. `tests/run_tests.sh --parallel`
runs the ten tests side by side after the import and prints the same lines in the same
order. What each test checks is in the main `README.md`.
...
test's `/tmp/fd-3T-battery-<pid>/` and the wear test's `/tmp/fd-3L-wear-<pid>/`, one
per process each, removed when the test finishes.
