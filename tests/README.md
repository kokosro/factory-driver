...
camera and mission tests, all on the tick clock (`--fixed-fps 60`), about half a minute.
`tests/run_tests.sh --parallel` runs the five tests side by side after the import and
...
anywhere else. Several copies can be gated at once: the only thing the suite writes
outside the project is the smoke test's `/tmp/fd-3R-smoke-<pid>/`, one per process,
