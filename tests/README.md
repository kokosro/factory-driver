...
skeleton, smoke, handling, camera, mission, battery, thermal, tyre/brake thermal,
steering-feel, wear, licence, menu and airborne tests, the driving ones on the tick clock
(`--fixed-fps 60`), under a minute.
`tests/run_tests.sh --parallel` runs the fifteen tests side by side after the import and
